...
case $n in *sh|"${0##*/}");;*[Ll]ogin*|*init|*systemd*)break;;*)term=$n;esac
done
[ "$wm" ]||for i in /proc/*/comm;do read -r c<"$i"
case $c in *bar*|*rc);;awesome|xmonad*|qtile|sway|i3|[bfo]*box|*wm*)wm=${c%%-*};break;esac
done
while read -r l;do eq "$l" 'MemTotal*'&&set -- $l&&break;done</proc/meminfo
mem="$(( $2 / 1000 ))MB"
while read -r l;do case $l in vendor_id*)v="${l##*: } ";;model\ name*)cpu=${l##*: };break;esac;done</proc/cpuinfo
IFS=. read -r u _ </proc/uptime;d=$((u/60/60/24));up=$(printf %02d:%02d $((u/60/60%24)) $((u/60%60)))
[ "$d" -gt 0 ]&&up="${d}d $up"
...
set --
for i in '/var/db/kiss/installed/*' '/var/lib/pacman/local/[0-9a-z]*' '/var/lib/dpkg/info/*.list' '/var/db/xbps/.*' '/var/db/pkg/*/*';do
set -- $i;[ $# -gt 1 ]&&pkgs=$#&&break
done
read -r h </proc/sys/kernel/hostname
elif [ -f /var/run/dmesg.boot ];then
read -r b </var/run/dmesg.boot
case $b in Open*)while read -r l;do case $l in 'real mem'*)mem=${l##*\(};mem=${mem%\)*};;cpu0:*)cpu=${l#cpu0: };cpu=${cpu%%,*};v=${cpu%%[\( ]*};break;;*)[ "$ID" ]||{ set -- $l;ID="$1 $2"; };esac;done</var/run/dmesg.boot
[ -d /var/db/pkg ]&&set -- /var/db/pkg/*&&pkgs=$#
read -r h </etc/myname;h=${h%.*};;*) . /etc/rc.conf;h=${hostname:-}
while read -r l;do case $l in FreeBSD*)[ "$ID" ]&&continue;ID=${l%%-R*};;CPU:*)cpu=${cpu#CPU: };cpu=${l%\(*};;*Origin=*)v=${l#*Origin=\"};v="${v%%\"*} ";;'real memory'*)mem=${l##*\(};mem=${mem%\)*};break;esac;done</var/run/dmesg.boot
esac
elif v=/System/Library/CoreServices/SystemVersion.plist;[ -f "$v" ];then
t=;while read -r l;do case $l in *ProductVersion*)t=.;;*)[ "$t" ]||continue;ID=${l#*>};ID="MacOS ${ID%<*}";break;esac;done<"$v"
fi
...
case $i in n)echo;;os)p os "$ID";;sh)p sh "${SHELL##*/}";;wm)p wm "${wm##*/}";;up)p up "$up";;gtk)p gtk "${gtk# }";;cpu)p cpu "$v$cpu";;mem)p mem "$mem";;host)p host "$m";;kern)p kern "$kernel";;pkgs)p pkgs "$pkgs";;term)p term "$term";;user)printf '%7s@%s\n' "$USER" "$h";;col)col;esac
done
